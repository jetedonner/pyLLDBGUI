...
void subfunc(int idx) {
  printf("\n%d", idx);
  double real_elapsed_time = difftime(time(NULL), start_real_time);
  printf("\nElapsed real time using time(): %.2f seconds\n", real_elapsed_time);
}
...
//clock_t start_time = clock(); // Get starting clock ticks
...
//    double elapsed_time = (double)(clock() - start_time) / CLOCKS_PER_SEC;
      ...
      // Print the results
//    printf("Elapsed time using clock(): %.2f seconds\n", elapsed_time);
